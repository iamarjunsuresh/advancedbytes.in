...
function set_theme(){
	...
	$("#dynamic-css").attr("href", "/gate2018assets/gatestyles-" + mytheme + ".css");
}
function submitURL(){
...
// ##todo arjun  add other subjects


	var uid_regex = /Participant ID:.*([E,C]{2,2})(\w+)/;
...
function log_marks(){
    ...
    $.post("/gate2018logmark/",
    	{id: window.uid, mark: window.final_marks.toFixed(2),branch:window.branch,ans:window.givans},
    	function(data){
			console.log(data);
		});
...
function process(){
	...
	var img_regex = /_(e|g)(?:e|a\d)_(\d\d)\.jpg$/;
	var q_regex = /^Question ID :(\d{10})Status :(Answered|Not Answered|Marked For Review|Not Attempted and Marked For Review)(C|G)(?:hosen Option|iven Answer) :(.*)$/;
...
//console.log(found_img,found_ttext,table_text);	
...
console.log(found_ttext);
			q_isneg = (found_ttext[3] === "C")?-1.0:0.0;
			given_answer = found_ttext[4];
			...
			givenAnswers[q_short_id] = {
				...
				"short_id": q_short_id,
				"long_id": q_long_id,
				"is_neg": ((q_no>=17&&q_no<=25) ||q_no>=40)?0:-1,
	...
	var myset = window[window.branch]["set1"];
	console.log(myset);
	window.givans= JSON.stringify(givenAnswers);
;
		...
		if(givenAnswers.hasOwnProperty(qsid)){
			...
			if(q.answer > -9000){
				...
				if(fuzzyEquality(q.answer, kq)){
					...
					q.node.find('tr:last').after('<tr style="color: green;"><td align="right">✔</td><td class="bold">+' + marks.toFixed(2) + '</td></tr><tr><td colspan="2" align="center"><a href="' + kq[1] + '"></a></td></tr>');
				}
				else{
					marks_negative += neg_marks;
					if(is_neg==0 &&marks==2.0)

{
				q.node.find('tr:last').after('<tr style="color: red;"><td align="right">✗</td><td class="bold">' + neg_marks.toFixed(2) + '</td></tr><tr><td align="right">Correct Answer:</td><td class="bold">'+kq[0] +' to '+kq[2]+'</td></tr><tr><td colspan="2" align="center"><a href="' + kq[1] + '"> </a></td></tr>');
					...
					q.node.find('tr:last').after('<tr style="color: red;"><td align="right">✗</td><td class="bold">' + neg_marks.toFixed(2) + '</td></tr><tr><td align="right">Correct Answer:</td><td class="bold">'+kq[0]+'</td></tr><tr><td colspan="2" align="center"><a href="' + kq[1] + '"> </a></td></tr>');
	}			}
			}
			else{
				// alert(q.long_id);
				console.log(q.long_id);
				q.node.find('tr:last').after('<tr><td colspan="2" align="center"><a href="' + kq[1] + '"></a></td></tr>');
			}
		}
	}
